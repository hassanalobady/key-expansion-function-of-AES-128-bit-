/*
	 * NEW KEY EXPANSION FUNCTION OF RIJNDAEL 128-BIT.
	 *
	 * (c) 2017 Hassan Mansour
	 *
	 * - key size = 128
	 * - number of rounds = 10
	 */


#include "iostream"
#include <math.h>
#define KEY_SIZE 256
#define N_KEYS 16
#define EXPANDED_KEY_COUNT 44


unsigned int Rcon[10] = { 0x01000000, 0x02000000, 0x04000000, 0x08000000, 0x10000000, 0x20000000, 0x40000000, 0x80000000, 0x1B000000, 0x36000000 };

unsigned char rcon[KEY_SIZE] = {
	
};

unsigned int RotWord(unsigned int w)
{


	int MSByte = w >> (32 - 16);


	w <<= 16;


	w += MSByte;

	return w;
}



unsigned int SubWord(unsigned int w)
{

	unsigned char words[4];

	for (int i = 0; i<4; i++)
	{
		words[3 - i] = w % (int)pow(2, 8);
		w >>= 8;
	}

	for (int i = 0; i< 4; i++)
	{

		int row = (words[i] >> 4);
		int col = words[i] % 16;

		unsigned char res = rcon[16 * row + col];

		w += res;
		if (i != 3)
			w <<= 8;
	}


	return w;
}



void KeyExpansion(unsigned char key[N_KEYS], unsigned int* w)

{

	unsigned	int temp;
	signed int temp1;
	for (int i = 0; i< N_KEYS; i++)
	{
		w[i] = (key[N_KEYS*i] << 24) + (key[N_KEYS*i + 1] << 16) + (key[N_KEYS*i + 2] << 8) + key[N_KEYS*i + 3];

	}

	for (int i = 4; i< EXPANDED_KEY_COUNT; i++)
	{
		temp = w[i - 1];
		if (i % 4 == 0)
			temp = SubWord(RotWord(temp)) ^ Rcon[i / 4];

		else (i % 4 == 4);
		temp1 = w[i - 4];
		temp1 = SubWord(temp1);
		w[i] = temp1 ^ temp;


	}
}


int main(int argc, const char * argv[])

{


	unsigned char key[N_KEYS] = { 0x7d ,0x95, 0x58 ,0xb1 ,0x99 ,0x78 ,0x95 ,0x2f ,0xde ,0x55 ,0x73 ,0x97 ,0xeb ,0xb2, 0xa7, 0xab };


	unsigned int* w = new unsigned int[EXPANDED_KEY_COUNT];

	KeyExpansion(key, w);
	/*int count = 1;*/
	for (int i = 4; i<EXPANDED_KEY_COUNT; i = i + 4)
	{
		std::cout  /*<< "K"*/ << std::dec /*<< count << ": " */ << std::hex << w[i] << w[i + 1] << w[i + 2] << w[i + 3] << std::endl;
		/*count++;*/
	}
	int a;
	std::cin >> a;
	return 0;
}
